import { WorkHeroProps } from "@/data/workpageData";
import Link from "next/link";

export default function WorkHero({
    title, highlightedText, description, buttonText, buttonLink, bgColor
}: WorkHeroProps) {
    return (
        <section className={`relative h-screen w-full overflow-hidden flex items-center px-10 justify-center md:justify-start ${bgColor}`}>
                ...
                <div className="mt-8 flex flex-col md:flex-row gap-4">
                    <Link href={buttonLink}>
                        <button className="group relative overflow-hidden p-1 cursor-pointer flex items-center justify-center gap-2 rounded-full bg-white text-black">
                            <span className="relative flex flex-col text-[14px] font-medium overflow-hidden pl-4 pr-1 h-5">
